...
{
    float timeShot = 0.25f* Time.deltaTime;
        ...
        else
        {
            ...
            if (angle < fov)
            {
                if (timeShot > 0.25f)
                {
					...
					Ammo -+ Ammo; // lower the ammo, if ammo is equal to zero seek ammo point.
					if (Ammo = 0)
					{
						myGameObject.GetComponent<SteeringBehaviours>().AmmoCollectionEnabled = true;
					}
                }
            }
        }
